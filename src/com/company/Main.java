...
public class Main {
    ...
    public static void main(String[] args) {
        ArrayList<Qubit> values = new ArrayList<>();
        ArrayList<String> names = new ArrayList<>();

        for (int i = 0; i < 10; i++){
            Qubit q = new Qubit();
            q.setState(new QState(1,0));
            q.setId("q"+(3*i));
            values.add(q);
            names.add(q.getId()+"_0");
            names.add(q.getId()+"_1");

            q = new Qubit();
            q.setState(new QState(0,-1));
            q.setId("q"+(3*i+1));
            values.add(q);
            names.add(q.getId()+"_0");
            names.add(q.getId()+"_1");
        ...
        MyHistogram h = new MyHistogram(values,names,"title");
//        h.paintComponent(new Graphics() {
//            @Override
